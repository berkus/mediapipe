...
if [ -z "$1" ]
  then
    ...
    cd release
    rm -rf *
    emcmake cmake .. -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=$(pwd)/opencv_wasm \
          -DOPENCV_FORCE_3RDPARTY_BUILD=YES -DBUILD_opencv_ts=YES -DBUILD_opencv_js=YES \
          -DBUILD_TESTS=OFF -DBUILD_PERF_TESTS=OFF \
          -DOPENCV_EXTRA_MODULES_PATH=/tmp/build_opencv/opencv_contrib/modules \
          -DBUILD_opencv_aruco=OFF -DBUILD_opencv_bgsegm=YES -DBUILD_opencv_bioinspired=OFF \
          ...
          -DBUILD_opencv_line_descriptor=OFF -DBUILD_opencv_phase_unwrapping=OFF \
    ...
    emmake make install
    rm -rf /tmp/build_opencv
    echo "OpenCV has been built. You can find the header files and libraries in $(pwd)/opencv_wasm/include/opencv2/ and $(pwd)/opencv_wasm/lib"
